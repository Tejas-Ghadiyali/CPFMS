...
router.post('/', middleware.loggedin_as_superuser ,(req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { dist_id, dist_name } = req.body;
            dist_id = dist_id.trim();
            var sql = 'INSERT INTO `District` (`dist_id`, `dist_name`) VALUES (?, ?)'
            connection.query(sql, [dist_id, dist_name], 
                (err, result) => {
                connection.release();
                if (err) {
                    console.log(err);
                    if (err.code == 'ER_DUP_ENTRY')
                        req.flash('danger', 'Distrcit with District Id ' + dist_id + ' already exists!');
                    else
                        req.flash('danger', 'Error while adding district in Master-Distrit!');
                    res.redirect('/district');
                }
                else {
                    req.flash('success', 'District with District Id ' + dist_id + ' Added.');
                    res.redirect('/district');
                }
            });
        }
...
router.post('/edit', middleware.loggedin_as_admin ,(req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { dist_id, dist_name } = req.body;
            dist_id = dist_id.trim();
            var sql = " UPDATE `District` SET `dist_name` = ? WHERE `District`.`dist_id` = ? ";
            connection.query(sql, [dist_name, dist_id], (err, results) => {
                connection.release();
                if (err) {
                    req.flash('danger', 'Error while editing record with id ' + dist_id);
                    console.log(err);
                    res.redirect('/district');
                }
                else {
                    req.flash('success', 'Successfully edited record with id ' + dist_id);
                    res.redirect('/district');
                }
            });
        }
...
router.post('/delete', middleware.loggedin_as_admin ,(req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var sql = "DELETE FROM `District` WHERE dist_id IN (?)";
